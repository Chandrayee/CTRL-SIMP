from preprocessing import decompose_input, make_input_string, decompose_slots, get_training_data
...
def batch_generator(textpairs, slots, all_annotation, batch_size=16):
  batch = []
  for i in range(len(textpairs)):
    inputs, outputs = get_training_data(all_annotation[i], slots[i]) 
...
def get_eval_data(textpairs, slots, all_annotation):
  batch = []
  for i in range(len(textpairs)):
    inputs, outputs = get_training_data(all_annotation[i], slots[i]) 
    #option 1: treating every example equally - keep a max combination, repeat example that do not have enough combination
