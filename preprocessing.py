...
def process_del_punct(s):
  ...
  for token, dem in all_patterns:
    ...
    if (len(tokens_tag) == 1 and 'NN' not in tags and 'JJ' not in tags and 'RB' not in tags and 'VB' not in tags and 'JJS' not in tags) or (re.fullmatch('['+string.punctuation+']+', pos_string[0][0])):
...
def available_slots(texts):
  ...
  """
  patterns = ["<rep>", "<elab-define>", "<elab>", "<elab-sentence>", "<del>"]
  ...
  for pattern in patterns:
    if pattern != "<elab-sentence>":
      if pattern == "<del>":
        _, pattern_list = process_del_punct(s)
        if len(pattern_list)>0:
          pattern_list = [['<del>', 'None'] + list(x) + [(-1, -1)] for x in pattern_list]
        ...
        pattern_list = extract_pattern(pattern, s)
        if len(pattern_list)>0:
          all_patterns += pattern_list
          annotated = True
          input_slots.add(ANNOTATION_SLOT_MAP[pattern])
      ...
      pattern_list = get_added_sentence(pattern, s)
      if len(pattern_list)>0:
...
def get_balanced_data(complete_input_slots, complete_output_slots, complete_annotation_dict):
    '''
    ...
    print('R: {}, D: {}, X: {}'.format(len(raw_data['R']), len(raw_data['D']), len(raw_data['X'])))
...
def prep_final_slots_per_datapoint(textpairs, sampling = False):
  ...
  for i, (inputs, outputs, annotations) in enumerate(textpairs):
    slots[i].append((['E'],['S']))
    control_slots = inputs.intersection(set(['X','C','R','F','Xs', 'D']))
  ...
  for i in E_S:
    slots[i].append((['E'],['S']))
    control_slots = all_inputs[i].intersection(set(['X','C','R','F','Xs']))
    slots[i].append((['E']+ list(control_slots),['S']))
    slots[i].append((['E'],['S'] + list(control_slots)))

  return all_inputs, all_outputs, all_annotations, slots

def get_training_data(annotation_dict, slot):
  inputs = []
  outputs = []
  for inp, out in slot:
    inpcopy = inp.copy()
    ...
    for more_slot in inpcopy:
      if more_slot in annotation_dict:
        input_string += more_slot + ':' + str(annotation_dict[more_slot]) 
    input_string += "\n"+"\n".join(out)
...
def load_data(textpairs):
  complete_input_slots, complete_output_slots, complete_annotation_dict = get_annotations(textpairs)
  textpairs = get_balanced_data(complete_input_slots, complete_output_slots, complete_annotation_dict)
...
if __name__ == '__main__':
    ...
    print(len(textpairs))
    textpairs = textpairs[:10]
    textpairs, all_inputs, all_outputs, all_annotations, slots = load_data(textpairs)
